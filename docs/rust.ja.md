...
指標の名前は，何を測ったのかを述べていない．リポジトリに `runvault.toml` があれば
`Run::start` がそれを読み，`finish()` がこの run の名前に当たる分だけを
`metrics.meta.json` へ写す．

```toml
[metrics."segregation_index"]
meaning   = "格子が混ざった状態からどれだけ離れているか"
unit      = "ratio"
direction = "down"
range     = [0.0, 1.0]
```

読むのは `RunOptions::repo_root` に渡した根，または `origin = code` のときの git の
根であり，読むのは開始時の 1 度だけである．走行中に編集されたファイルに «その run が
何を測ったか» を決めさせないためである．宣言を持たないリポジトリでも run は記録できる．
生成された名前の一族の書き方と，誰も宣言しなかった分を数える監査については
...
ディレクトリ名は «いつ» と «どの条件» は言うが，その条件が **何のためのものか**
は言わない．`label` がそれである．
...
`label` はどのハッシュもパスも変えない —— 同じ条件に別の名前を付けた 2 本はやはり
同じ条件であり，ディレクトリは runvault が付けた名前のままである．`sweep_point` は
グリッドの何番目か（0 始まり）と点の総数を記録するので，画面が «7/40» と言える．

番号を呼び出し側が渡すのは，**ループを回すのが呼び出し側だから**である．runvault が
既存の run を数えて振った番号は同じもののように見えて違う —— 2 点が同時に走れば同じ
番号になり，1 点が失敗すればそれ以降が全部ずれる．番号を持つ run は sweep に属して
いなければならない（sweep の外に «7 番目» になれるグリッドは無い）．

**名前には条件そのものを書き，ループ上の位置は書かない．** 位置は `sweep_index` が
持っているので，名前に番号を書くとグリッドを変えるたびに両方を直すことになる．
