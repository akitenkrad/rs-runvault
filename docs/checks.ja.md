...
- **`artifacts/` を再ハッシュする**．これが，生成ファイルが記録の外で生きるのを止める．ただし «ディスク上にあって `manifest.csv` に載っていないファイルが無いか» を問う側の半分は，manifest があることを前提としている．manifest を書くのは `finish()` だけなので，**failed として記録され，かつ manifest を持たない run** はこの半分の対象外とする．そうした run は kill されたか drop されたか start で失敗したかであり，`artifacts/` に残っているものは途中で切れた書き込みの残骸である —— 後からそれを manifest に載せる工程は存在しない．封をした run は `finished` でも `failed` でも従来どおり自分の生成物に責任を負う．なお «manifest が無いこと» は `finish()` の書き込み順序からの推定であって証明ではない —— run ディレクトリを書き換えられる者は同じ形を作れる．`verify` は «その run が自分自身と矛盾していないか» を見る検査であり，事故を捕まえるためのものである．記録は署名されていないので，敵対的な改ざんの検出は主張しない．
