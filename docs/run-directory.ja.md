...
`label` はその run を人が何と呼んだか（«τ=1/3 基準条件»）．任意で，これができる前に
記録された run にはどれも無く，どのハッシュにも入らない —— 同じ条件に別の名前を付けた
2 本はやはり同じ条件である．ディレクトリ名にも入らない．`lineage.sweep_index` /
`sweep_total` は sweep のグリッドの何番目か（0 始まり）を表す．
